...
| Pestaña del navegador mostraba sólo el folio (`PT-2026...`), sin marca | El título ponía el código adelante; una pestaña truncada no mostraba nada reconocible. Primer intento (marca primero, folio al final en el mismo título) resolvía la pestaña pero el usuario no quería el folio ahí en absoluto. Solución final: título fijo `"ALTO TEST — Propuesta Técnica"` siempre, y sólo durante la impresión (`beforeprint`/`afterprint`, cubre el botón y Ctrl+P) se cambia al folio — que es lo que Chrome toma como nombre de archivo al "Guardar como PDF". Verificado que el título real en el instante de `beforeprint` es el folio (headless dispara ambos eventos sincrónicos, sin pausa real de diálogo, así que hay que loguear *dentro* del handler para verlo) |
